...
import com.kjrepo.infra.runner.rpc.grpc.GrpcClient;
import com.kjrepo.infra.runner.rpc.grpc.GrpcInfo;
import com.kjrepo.infra.runner.rpc.grpc.GrpcInfoItem;
...
public class Grpc2Client {
	...
	public Grpc2Client(String key, Class<?> clazz) {
		this.cluster = ClusterFactory.gcluster(
				GroupRegisterFactory.getContext().getGroupRegister(GrpcInfo.class, GrpcInfoItem.class), key,
				info -> ManagedChannelBuilder.forAddress(((InstanceInfo<GrpcInfoItem>) info).getInfo().getHost(),
						((InstanceInfo<GrpcInfoItem>) info).getInfo().getPort()).usePlaintext().build(),
